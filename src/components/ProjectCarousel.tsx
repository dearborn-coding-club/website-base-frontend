...
import ProjectCarouselCard, { ProjectDetailsProp } from "./ProjectCarouselCard"
import ProjectCarouselCardSkeleton from "./ProjectCarouselSkeleton"
...
const ProjectCarousel: React.FC<{prop: ProjectCarouselProp}> = (carouselProp) => {

    const {carouselCardWidth = 320, gap = 10} = carouselProp.prop

    const [projectList, setProjectList] = useState<ProjectDetailsProp[] | string[]>(["","","","",""])
    ...
    const style = { "--carousel-card-width": `${carouselCardWidth}px`, "--gap": `${gap}px` } as React.CSSProperties
    ...
    if(isLoading) {
        return (
            <div className="ProjectCarousel" style={style}>
                ...
                <div className="card-wrapper" style={{justifyContent: isInfiniteScroll?"start":"center"}} ref={cardWrapperRef} >
                    {isInfiniteScroll && projectList.map((_, i) => <ProjectCarouselCardSkeleton key={i}/>)}
                    <div ref={listRef} className="card-list">
                        {projectList.map((_, i) => <ProjectCarouselCardSkeleton key={i+projectList.length}/>)}
                    </div>
                    {isInfiniteScroll && projectList.map((_, i) => <ProjectCarouselCardSkeleton key={i+projectList.length*2}/>)}
                </div>
    ...
    return (
        <div className="ProjectCarousel" style={style}>
            {isInfiniteScroll && <button type="button" className="left-arrow" onClick={handleOnLeftArrowClick}> ← </button>}
            <div className="card-wrapper" style={{justifyContent: isInfiniteScroll?"start":"center"}} ref={cardWrapperRef} >
                {isInfiniteScroll && projectList.map((e, i) => <ProjectCarouselCard key={i} projectDetails={e as ProjectDetailsProp}/>)}
                <div ref={listRef} className="card-list">
                    {projectList.map((e, i) => <ProjectCarouselCard key={i+projectList.length} projectDetails={e as ProjectDetailsProp}/>)}
                </div>
                {isInfiniteScroll && projectList.map((e, i) => <ProjectCarouselCard key={i+projectList.length*2} projectDetails={e as ProjectDetailsProp}/>)}
            </div>
            {isInfiniteScroll && <button type="button" className="right-arrow" onClick={handleOnRightArrowClick}> → </button>}
        </div>
    )
